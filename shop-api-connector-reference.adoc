...
The Shop API Connector is an Operation Based Connector, which means that when you add the connector to your flow, you need to configure a specific operation the connector is intended to perform. Shop API Connector will support 3 different types of configuration and the details are given below.
...
| Base Uri a| String |  Parameter base URI, each instance/tenant gets its own |  default base URI | {nbsp}
| Client Id a| String |  client_id |  | *x*{nbsp}
| Type a| Enumeration, one of:

** CREDENTIALS
** GUEST |  Customer type: registered customer or guest customer |  | *x*{nbsp}
| Protocol a| Enumeration, one of:
